...
decision_boundary <- x_new |>
  filter(p1 > 0.5) |>
  slice(1) |> # take closest x where p is above 0.5
  pull(x)
...
# Choose two extreme points along x1 to draw the straight decision boundary line
# The decision boundary occurs where the linear predictor equals 0 (due to logit a = 0)):
#   b[1] + b[2]*x1 + b[3]*x2 = 0
x1_boundary <- c(2.9, 7)
