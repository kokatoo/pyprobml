...
max_log_lik <- Nh * log10(mle + eps) +
  Nt * log10(1 - mle + eps)

# Log marginal likelihood (natural log)
log_marginal <- lbeta(alphaH + Nh, alphaT + Nt) -
  lbeta(alphaH, alphaT)

log10_marginal <- log_marginal / log(10)

# BIC approximation to log marginal likelihood (natural log)
k <- 1  # one parameter (theta)
bic_approx <- max_log_lik - (k / 2) * log(N)
