...
generate_plots <- function(N) {
  ...
  for (deg in deg_values) {
    ...
    SN_inv <- S0_inv +
      eb$beta * t(X_train) %*% X_train
    SN <- solve(SN_inv)
    mN <- SN %*%
      (S0_inv %*% m0 +
         eb$beta * t(X_train) %*% y_train)

    y_mean <- as.vector(X_true %*% mN)
    y_std <- sqrt(
      1/eb$beta +
        rowSums((X_true %*% SN) * X_true)
    )
    ...
    p <- ggplot(
     ...
     aes(x = x)
      ...
      geom_line(
        aes(y = upper), linetype = "dotted"
      ) +
      geom_line(
        aes(y = lower), linetype = "dotted"
      ) +
      geom_line(
        aes(y = mean), color = "red"
      ) +
      geom_point(
        data = tibble(
          x = x_train,
      ...
      ggtitle(
        paste0(
          "deg=", deg,
          " | log p(D|M)=", round(log_p_D_M_val, 2)
        )
      ) +
      theme_minimal()
